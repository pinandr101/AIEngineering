...
def report(
    ...
    min_missing_share: float = typer.Option(0.05, help="Порог допустимой доли пропусков"),
) -> None:
    """
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- Есть дублирующиеся идентификаторы: **{quality_flags["has_suspicious_id_duplicates"]}**\n")
        f.write(f"- Есть числовые колонки с слишком большим содержанием нулевых значений: **{quality_flags["has_many_zero_values"]}**\n\n")
        ...
        f.write(f"Проблемные колонки с слишком большим содержанием пропусков (больше {min_missing_share}) ")
        if (missing_df.iloc[0]["missing_share"] <= min_missing_share):
            f.write("найдены не были.\n\n")
        else:
            f.write("были найдены: \n")
            for i in range(len(missing_df)):
                if missing_df.iloc[i]["missing_share"] > min_missing_share:
                    f.write(f"{missing_df.index[i]}\n")
                else: 
                    break
            f.write("\n\n")
        ...
        if not top_cats:
            ...
            f.write(f"См. топ {top_k_categories} категориальных признаков в файлах в папке `top_categories/`.\n\n")

        f.write("## Гистограммы числовых колонок\n\n")
        f.write(f"Максимальное число колонок для гистограмм: {max_hist_columns}\n")
...
def head(
    path: str = typer.Argument(..., help="Путь к CSV-файлу."),
    n: int = typer.Option(5, help="Количество первых n строк для вывода"),
    sep: str = typer.Option(",", help="Разделитель в CSV."),
    encoding: str = typer.Option("utf-8", help="Кодировка файла."),
) -> None:
    """
