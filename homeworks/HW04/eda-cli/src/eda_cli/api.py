...
@app.post(
    ...
    tags=["quality"],
    summary="Возрат флагов качества для CSV-файла с использованием EDA-ядра",
)
def quality_flags_from_csv(file: UploadFile = File(...)) -> dict:
    """
    Эндпоинт, который принимает CSV-файл, запускает EDA-ядро
    (summarize_dataset + missing_table + compute_quality_flags)
    и возвращает JSON, который содержит словарь флагов качества
    и их значений.
    """
    if file.content_type not in ("text/csv", "application/vnd.ms-excel", "application/octet-stream"):
        raise HTTPException(status_code=400, detail="Ожидается CSV-файл (content-type text/csv).")
...
@app.post(
    "/head",
    tags=["data"],
    summary="Возвращает заданное количество первых строк датасета в формате JSON; По умолчанию n=5."
)
def head_csv(
