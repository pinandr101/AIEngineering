...
def _constant_column_df() -> pd.DataFrame:
    return pd.DataFrame(
        {
            "model": ["BMW", "Shkoda", "Toyota"],
            "color": ["red", "orange", "black"],
            "isElectric": [True, True, True],
        }
    )

def _suspicious_id_df() -> pd.DataFrame:
    return pd.DataFrame(
        {
            "id": [1, 2, 3, 3, 4],
            "name": ["Andrew", "Alex", "Boris", "John", "Paul"],
            "income": [60000, 70000, 750000, 80000, 65000],
        }
    )
